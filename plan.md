...
1.  **Fit the Background Noise Model:**
    *   As is currently done, perform a robust regression on the log-power vs. log-frequency plot.
    *   **For a standard fit:** This model is the single fitted line: `log(P) = -β * log(f) + c`.
    *   **For a segmented fit:** This model is the full piecewise regression model, consisting of two lines and a breakpoint.

2.  **Calculate Significance from Residuals:**
    *   For every point in the periodogram, calculate the residual, which is the difference between the observed log-power and the log-power predicted by the background model.
    *   `residual = log(actual_power) - log(fitted_power)`

3.  **Establish a Confidence Interval:**
    *   The collection of all residuals forms a distribution representing the expected variance around the fitted background.
    *   A confidence interval can be established based on this distribution. For example, a 95% confidence level would be the 95th percentile of the residuals.
    *   This creates a confidence band that runs parallel to the fitted background slope(s).

4.  **Identify Significant Peaks:**
    *   A peak is considered statistically significant if its residual exceeds the confidence level. That is, if its power is improbably high compared to the background noise at that frequency.

### Benefits of this Approach

*   **Agnostic to Noise Color:** This method works for any `beta` value because it uses the observed `beta` to define the background. It does not assume the noise is white.
*   **Extends to Multifractal Analysis:** It works seamlessly with segmented fits by using the piecewise model as the background.
*   **Scientifically Robust:** It is a more reliable and data-driven method that aligns with the approaches used by established tools in the field.
