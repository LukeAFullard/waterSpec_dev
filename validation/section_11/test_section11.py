...
def run_test_11_2():
    ...
    for trial in range(N_TRIALS):
        ...
        short_passed = ac_surr_short > 0.4
        long_passed = np.abs(ac_surr_long) < 0.3
...
def run_test_11_3():
    ...
    for trial in range(N_TRIALS):
        ...
        beta_passed = np.abs(beta_surr - beta_orig) < 0.3
...
def run_test_11_4():
    ...
    for trial in range(N_TRIALS):
        ...
        beta_passed = np.all(np.abs(np.array(betas) - target_beta) < 0.3)
        if beta_passed:
            passed_trials += 1
