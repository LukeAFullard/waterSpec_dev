...
def generate_synthetic_series_with_peak(beta, signal_amp, seed=42):
    ...
    noise_fft = np.sqrt(power_spectrum) * np.exp(1j * rng.uniform(0, 2 * np.pi, len(freq)))
...
def run_single_validation(beta, signal_amp, temp_dir):
    ...
    time, series = generate_synthetic_series_with_peak(beta, signal_amp, seed=int(beta*10 + signal_amp*10))
    ...
    with open(os.devnull, 'w') as f, contextlib.redirect_stdout(f):
        ...
        try:
            ws_analyzer_resid = Analysis(file_path, time_col='time', data_col='value', detrend_method=None)
            ws_results_resid = ws_analyzer_resid.run_full_analysis(
                output_dir=os.path.join(temp_dir, 'resid'), grid_type='linear', peak_detection_method='residual'
            )
            ws_resid_found = False
            if 'significant_peaks' in ws_results_resid and ws_results_resid['significant_peaks']:
        ...
        try:
            ws_analyzer_redfit = Analysis(file_path, time_col='time', data_col='value', detrend_method=None)
            ...
            peaks_95ci = ws_results_redfit.get('significant_peaks_by_ci', {}).get(95, [])
            if peaks_95ci:
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Run a single waterSpec validation case.")
    parser.add_argument('--beta', type=float, required=True, help='Beta value for noise generation.')
    parser.add_argument('--amplitude', type=float, required=True, help='Amplitude of the injected signal.')
    ...
    try:
        ...
        print(f"| {beta:<6.1f} | {amp:<10.2f} | {ws_resid_str:<22} | {ws_redfit_str:<20} |")
