...
def generate_synthetic_series_with_peak(beta, signal_amp, seed=42):
    ...
    noise = np.fft.irfft(np.sqrt(power_spectrum) * np.exp(1j * rng.uniform(0, 2 * np.pi, len(freq))), n=N_POINTS)
...
def run_single_validation(beta, signal_amp, temp_dir):
    ...
    # --- waterSpec Analysis ---
    try:
        ws_analyzer = Analysis(file_path, time_col='time', data_col='value', detrend_method=None)
        ...
        if 'significant_peaks' in ws_results and ws_results['significant_peaks']:
            for peak in ws_results['significant_peaks']:
                if abs(peak['frequency'] - signal_freq_hz) < (signal_freq_hz * 0.15): # 15% tolerance
                    ws_peak_found = True
                    break
    except Exception:
        ws_peak_found = "ERROR"
    ...
    try:
        ...
        with localconverter(robjects.default_converter + pandas2ri.converter) as cv:
            redfit_results = dplr.redfit(robjects.FloatVector(series), nsim=500, mctest=True)
    ...
    try:
        for beta in BETA_VALUES:
            for amp in AMPLITUDE_VALUES:
                ...
                results_data.append({
                    'beta': beta, 'amplitude': amp,
                    'waterSpec': "✅ Found" if ws_found else "❌ Not Found",
                    'dplR': "✅ Found" if dplr_found else "❌ Not Found"
                })
                print(f"{beta:<6.1f} | {amp:<10.2f} | {results_data[-1]['waterSpec']:<10} | {results_data[-1]['dplR']:<10}")
    finally:
