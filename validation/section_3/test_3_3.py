...
def run_test_and_record(test_id, seed, time, data, time_sub, data_sub, params_dict, beta):
    # Plotting for one seed per beta-fraction combo
    ...
    if plot_this:
        plt.tight_layout()
        plt.savefig(os.path.join(PLOTS_DIR, f"{test_id}_beta{beta}_frac{params_dict['missing_fraction']}.png"))
        plt.close()

def test_3_3_clustered():
    print("Running 3.3 Clustered...")
    fractions = [0.1, 0.3, 0.5, 0.7, 0.9]
    for beta in BETAS:
        for frac in fractions:
            for seed_idx in range(N_SEEDS):
                seed = get_seed(3, 3000 + int(beta*10) * 100 + int(frac*100) + seed_idx)
                np.random.seed(seed)
                time, data = generate_colored_noise(beta=beta, N=N_POINTS, seed=seed)

                # Clustered missingness
                n_points = len(time)
                n_missing = int(n_points * frac)
                n_blocks = max(1, int(n_missing / (n_points * 0.05))) # blocks of ~5% size
                block_size = n_missing // n_blocks

                mask = np.ones(n_points, dtype=bool)
                for _ in range(n_blocks):
                    start_idx = np.random.randint(0, n_points - block_size)
                    mask[start_idx:start_idx+block_size] = False

                time_sub = time[mask]
                data_sub = data[mask]

                params = {'beta': beta, 'missing_fraction': frac, 'method': 'clustered'}
                run_test_and_record("3.3_clustered", seed, time, data, time_sub, data_sub, params, beta)
