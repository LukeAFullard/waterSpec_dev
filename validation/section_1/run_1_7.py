...
def run_test(test_id, beta_target, amp, dt):
    print(f"\n--- Running Test {test_id} (Target Beta = {beta_target}, Amp = {amp}, dt = {dt}) ---")
    n_trials = 30
    ...
    for trial in range(n_trials):
        seed = get_seed(1, trial) + int(amp * dt * 1000)
        # Handle small rounding errors with integer conversions by just using a hash of the tuple if needed, or simple offset
        time, data = generate_colored_noise(beta_target, amp=amp, N=4096, dt=dt, seed=seed)
...
# Combinations of amplitude and dt
conditions = [
    (1.0, 1.0),
    (100.0, 1.0),
    (0.01, 1.0),
...
for amp, dt in conditions:
    run_test(f"1.7_amp{amp}_dt{dt}", 1.0, amp, dt)
