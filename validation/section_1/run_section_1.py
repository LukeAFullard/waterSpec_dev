...
def run_test(test_id, beta_target, exclude_low_freq=0):
    print(f"\n--- Running Test {test_id} (Target Beta = {beta_target}, exclude_low_freq = {exclude_low_freq}) ---")
    n_trials = 30
    ...
    for trial in range(n_trials):
        ...
        if exclude_low_freq > 0:
            freq_fit = freq[exclude_low_freq:]
            power_fit = power[exclude_low_freq:]
        else:
            freq_fit = freq
            power_fit = power
        ...
        record_result(f"{test_id}_OLS", seed, {'beta': beta_target, 'exclude': exclude_low_freq}, beta_ols, beta_target, ci_low_ols, ci_high_ols, ci_low_ols <= beta_target <= ci_high_ols, results_dir="validation/section_1/results")
        ...
        record_result(f"{test_id}_TS", seed, {'beta': beta_target, 'exclude': exclude_low_freq}, beta_ts, beta_target, ci_low_ts, ci_high_ts, ci_low_ts <= beta_target <= ci_high_ts, results_dir="validation/section_1/results")
...
run_test("1.2", 1.0)
run_test("1.3", 2.0)
run_test("1.3_exclude1", 2.0, exclude_low_freq=1)
run_test("1.3_exclude2", 2.0, exclude_low_freq=2)
run_test("1.4", -1.0)
run_test("1.5", -2.0)
