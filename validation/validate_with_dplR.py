...
def main():
    ...
    for res in results:
        ws_beta_str = (
            f"{res['waterspec_beta']:.4f}"
            if res["waterspec_beta"] is not None
            else "N/A"
        )
        dplr_beta_str = (
            f"{res['dplr_beta']:.4f}" if not np.isnan(res["dplr_beta"]) else "N/A"
        )
        row = (
            f"| {res['known_beta']:.2f}{' ':<7} | {ws_beta_str:<16} | "
            f"{dplr_beta_str:<12} |"
        )
        print(row)
    ...
    print("\n--- Comparison Summary ---")
    print(
        "This validation compares the spectral exponent (beta) estimated by "
        "`waterSpec`"
    )
    print(
        "with a beta estimated from the bias-corrected spectrum returned by "
        "`dplR::redfit`."
    )
    print(
        "The `dplR` beta is calculated by fitting a linear regression to the "
        "log-log spectrum"
    )
