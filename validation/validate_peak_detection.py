...
def generate_synthetic_series_with_peak(
    n_points=1024, beta=1.5, signal_freq=0.1, signal_amp=0.5, seed=42
):
    """
...
def main():
    ...
    dplr = importr("dplR")
    signal_freq_cpd = (
        0.1  # The known frequency of our injected signal in Cycles Per Day
    )
    signal_freq_hz = (
        signal_freq_cpd / 86400.0
    )  # Convert to Hz for comparison with waterSpec
    ...
    print(
        f"Injecting sine wave with freq={signal_freq_cpd} cycles/day "
        f"({signal_freq_hz:.2E} Hz), amp={signal_amp}"
    )
    print("waterSpec significance method: Residual from spectral fit (95% CI)")
    print("dplR significance method: 95% confidence interval from AR(1) simulations")
    print("-" * 50)
    ...
    try:
        ...
        if "significant_peaks" in ws_results and ws_results["significant_peaks"]:
            for peak in ws_results["significant_peaks"]:
                ...
                if abs(peak["frequency"] - signal_freq_hz) < (
                    ...
                    ws_peak_found = True
                    print(
                        f"  [SUCCESS] waterSpec found a significant peak at "
                        f"frequency {peak['frequency']:.2E} Hz"
                    )
                    break
        ...
        with localconverter(robjects.default_converter + pandas2ri.converter):
            ...
            if peak_power > peak_ci95:
                ...
                print(
                    f"  [SUCCESS] dplR found a significant peak at frequency "
                    f"{freq[peak_idx]:.4f} cycles/day"
                )
        ...
        if ws_peak_found and dplr_peak_found:
            print(
                "✅ SUCCESS: Both waterSpec and dplR identified the injected "
                "signal as significant."
            )
        else:
