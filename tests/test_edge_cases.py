...
def test_preprocess_with_loess_detrend(tmp_path):
    ...
    processed_data, _, _ = preprocess_data(series, time, detrend_method="loess")
...
def test_interpret_results_fap_peak_detection(tmp_path):
    ...
    interp = interpret_results(fit_results, param_name="Test")
    assert (
        "Significant Periodicities Found (at 1.0% FAP Level)" in interp["summary_text"]
    )
...
def test_find_peaks_via_residuals_no_fit(tmp_path):
    ...
    with pytest.raises(ValueError, match="fit_results is missing required keys"):
        find_peaks_via_residuals(fit_results, fdr_level=0.05)
...
def test_load_data_all_nan_data(tmp_path):
    ...
    with pytest.raises(
        ValueError, match="The data column 'value' contains no valid data."
    ):
        load_data(file_path, time_col="time", data_col="value", base_dir="")
...
def test_interpreter_formats_two_breakpoints(tmp_path):
    ...
    fit_results = {
        "betas": [0.5, 1.5, 0.8],
        "beta_ses": [0.1, 0.1, 0.1],
        "n_breakpoints": 2,
        "chosen_model": "segmented_2bp",
        "breakpoints": [0.1, 0.5],
        "significant_peaks": [],
    }
    interp = interpret_results(fit_results, param_name="Test")
