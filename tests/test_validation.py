...
def test_validate_ci_method():
    with pytest.raises(
        ValueError, match="`ci_method` must be 'bootstrap' or 'parametric'"
    ):
...
def test_validate_n_bootstraps():
    with pytest.raises(
        ValueError, match="`n_bootstraps` must be a non-negative integer"
    ):
...
def test_validate_fap_threshold():
    with pytest.raises(
        ValueError, match="`fap_threshold` must be a float between 0 and 1"
    ):
...
def test_validate_samples_per_peak():
    with pytest.raises(
        ValueError, match="`samples_per_peak` must be a positive integer"
    ):
...
def test_validate_fap_method():
    with pytest.raises(
        ValueError, match="`fap_method` must be 'baluev' or 'bootstrap'"
    ):
...
def test_validate_peak_fdr_level():
    with pytest.raises(
        ValueError, match="`peak_fdr_level` must be a float between 0 and 1"
    ):
