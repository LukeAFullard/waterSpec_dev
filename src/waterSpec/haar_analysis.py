...
def _compute_statistic(
    ...
    if statistic == "mean":
        ...
        if percentile is None:
            raise ValueError(
                "percentile must be provided when statistic is 'percentile'"
            )
...
def calculate_haar_fluctuations(
    ...
    for delta_t in lag_times:
        ...
        for i in range(len(t_starts)):
            ...
            if (
                len(vals1) >= min_samples_per_window
                and len(vals2) >= min_samples_per_window
            ):
                val1 = _compute_statistic(
                    vals1, statistic, percentile, percentile_method
                )
                val2 = _compute_statistic(
                    vals2, statistic, percentile, percentile_method
                )
                delta_f = val2 - val1
...
def fit_haar_slope(
    lags: np.ndarray, s1: np.ndarray, ci: float = 95, n_bootstraps: int = 100
) -> Dict:
    """
    ...
    res = MannKS.trend_test(
        log_s1, log_lags, alpha=1 - (ci / 100), n_bootstrap=n_bootstraps
    )
...
def plot_haar_analysis(
    ...
    plt.loglog(
        lags, s1, "o-", label="Haar Structure Function $S_1(\\Delta t)$", alpha=0.6
    )
    ...
    if not np.isnan(H) and segmented_results is None:
        ...
        plt.loglog(
            lags, fit_vals, "r--", label=f"Standard Fit: H={H:.2f}, $\\beta$={beta:.2f}"
        )
    ...
    if segmented_results and "Hs" in segmented_results:
        ...
        for bp in breakpoints:
            plt.axvline(
                bp, color="k", linestyle=":", alpha=0.5, label=f"Breakpoint: {bp:.1f}"
            )
    ...
    def run(
        ...
        if max_breakpoints > 0:
            ...
            for nb in range(1, max_breakpoints + 1):
                ...
                res = fit_segmented_haar(
                    self.lags, self.s1, n_breakpoints=nb, n_bootstraps=n_bootstraps
                )
