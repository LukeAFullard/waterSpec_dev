...
*   **Generalized Lomb-Scargle (GLS):** The standard LS periodogram inherently assumes that the data is centered around a known, true zero mean. In heavily irregular data, the sample mean can deviate significantly from the true underlying population mean. The Generalized Lomb-Scargle (GLS) periodogram, popularized by Zechmeister and Kürster (2009), resolves this by explicitly incorporating a floating mean parameter ($c$) into the sinusoidal fit: $y(t) = A \cos(\omega t) + B \sin(\omega t) + c$. This strictly prevents large-scale low-frequency artifacts caused by mean offsets from spuriously elevating high-frequency spectral power. `waterSpec` utilizes `astropy.timeseries.LombScargle`, which defaults to this mathematically superior generalized formulation (`fit_mean=True`), ensuring rigorous spectral estimates independent of baseline shifts.
...
$$ S_1(\tau) \propto \tau^m $$
This exponent $m$ relates directly to the spectral exponent $\beta$ via the equation $\beta = 2m + 1$ (Lovejoy & Schertzer, 2012). Mathematically, the Haar variance is analogous to half the Allan Variance ($\sigma_A^2$), a metric widely used to quantify the stability of oscillators, further validating its use as a robust estimator of multi-scale variance.
...
*   **Loop Area Significance:** In phase space, the area of the loop physically quantifies the magnitude of the hysteresis (the degree to which the system state depends on its trajectory, representing "memory" or non-equilibrium transport). Analogous to calculating the work done in a thermodynamic cycle, this area accurately measures the total deviation from linearity. The shoelace formulation elegantly captures both the overall magnitude and the prevailing temporal ordering.
*   **Directionality:** The sign of the computed area explicitly indicates the direction. A clockwise loop (positive area, often implying a rapidly depleted or proximal source) is strictly distinguished from a counter-clockwise loop (negative area, often implying a delayed, distant, or groundwater-driven source).
...
*   **The Autocorrelation Problem:** PELT and virtually all standard changepoint algorithms fundamentally assume that the model residuals (the data minus the fitted piecewise constant/linear model) are independent, identically distributed (i.i.d.) random variables. However, environmental time series are almost universally strongly autocorrelated (exhibiting red noise or long-range dependence).
*   **False Positives in Red Noise:** Applying standard changepoint detection to highly autocorrelated data will drastically and systematically inflate the false positive rate. Red noise processes naturally generate long excursions or "local trends" that masquerade exactly like structural mean shifts to the algorithm. Using PELT without rigorous pre-whitening or explicitly modeling the underlying autocorrelation structure (e.g., using AR-based cost functions) will identify "regime shifts" that are merely normal low-frequency stochastic excursions.
*   **Recommendation:** Strictly ensure data is appropriately pre-whitened or statistically robust against long-range dependence before interpreting structural changepoints in continuous environmental variables (Lund & Reeves, 2002).
...
1.  **Phase Randomization (FFT-based) & IAAFT:**
    ...
    *   *IAAFT (Iterative Amplitude Adjusted Fourier Transform):* A critical limitation of simple Phase Randomization is that while it perfectly preserves the power spectrum, it alters the fundamental probability distribution (amplitude distribution) of the underlying data, often forcing it toward Gaussianity. The Iterated Amplitude Adjusted Fourier Transform (IAAFT) mathematically resolves this by iteratively swapping amplitudes and phases until both the power spectrum and the exact original amplitude distribution are simultaneously preserved (Schreiber & Schmitz, 1996). This is strictly required when the original data is highly non-Gaussian, preventing the surrogate null distribution from being synthetically narrowed or skewed by artificial Gaussianization.
...
3.  **Bootstrapping Strategies in Spectral Fitting:**
    ...
    *   *Wild Bootstrapping:* `waterSpec` implements highly robust wild bootstrapping utilizing Rademacher distributions (randomly multiplying centered residuals strictly by $-1$ or $1$ with $p=0.5$). This is exceptionally defensible for datasets exhibiting severe structural heteroscedasticity, as it perfectly mathematically preserves the exact variance structure at each individual specific temporal or frequency index (Liu, 1988).
    *   *Moving Block Bootstrapping:* When intense non-linearities or explicit, long-range autocorrelations are present alongside irregular sampling, all aforementioned strategies mathematically fail. Moving block bootstrapping (Künsch, 1989) resolves this by resampling contiguous sequential chunks (blocks) of data rather than individual points. This critically and rigidly preserves the short-range empirical autocorrelation structure and non-linear dependencies inherently contained within the designated block size, while structurally scrambling long-range dependence, producing vastly more defensible standard errors in heavily autocorrelated environmental data.
